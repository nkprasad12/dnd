...
function getElementById(id: string): HTMLElement {
  ...
  if (element == null) {
    throw 'getElementById on invalid id: ' + id;
  }
  return element;
}

function setupModal(buttonId: string, modalId: string, closeId: string): void {
  let modal: HTMLElement = getElementById(modalId);
...
function handleImageUpload(event: HTMLInputEvent): Promise<string> {
  if (event?.target?.files == null || event.target.files.length == 0) {
    return Promise.reject('File upload event was null');
  }
  const file: File = event.target.files[0];
  console.log(file.type);
  if (!IMAGE_TYPES.includes(file.type)) {
    return Promise.reject('Invalid file upload type');
  }
  ...
  return fetch('http://127.0.0.1:5000/uploadImage', {
    ...
    .then(data => {
      let path = 'http://localhost:5000/retrieve_image/' + data.path;
      return path;
    });
}
  ...
  getElementById(CREATE_UPLOAD_BUTTON).addEventListener(
    ...
    (event) => {
      handleImageUpload((event as HTMLInputEvent))
        .then(imageSource => {
          // TODO: Validate that other fields were set correctly and create a RemoteBoardModel from this
